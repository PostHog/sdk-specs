...
Feature: Get Feature Flags And Payloads
  ...
  Scenario Outline: Single and bulk APIs isolate malformed payloads consistently
    Given the SDK is initialized with token "test-token"
    ...
    And no raw malformed string should be returned
    And no exception should be thrown by either call
