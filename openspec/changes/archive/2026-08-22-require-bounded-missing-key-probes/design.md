...
Concurrent evaluations whose only new missing-key work overlaps wait for one in-flight existence probe for that key. After the probe completes, waiters re-evaluate the state. A clean omission suppresses their duplicate probes. If the key is returned, a waiter with a different distinct id or property/group context must perform its own remote evaluation because the first response's value and payload are identity-specific. Only a complete cache hit for the same reusable evaluation context may avoid that request.
