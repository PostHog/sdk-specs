...
A current-generation response that explicitly returns a retained-missing key removes the contradictory omission immediately, including when another unresolved key caused the original-scope fallback. Omission learning is limited to the unknown keys assigned to the probe when it began. A key already suppressed at request start may appear in the original scope, but that request cannot refresh or reinstall its omission. This ownership rule prevents a delayed omission from overwriting newer positive evidence. A later call with the same identity and scope remains eligible to probe after an inconclusive response. Implementations with a general evaluated-result cache must bypass or distinguish cached inconclusive entries when that cache would suppress the required retry.
