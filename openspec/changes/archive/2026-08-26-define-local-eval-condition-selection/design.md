...
The checked-out backend reference is `/Users/db/dev/posthog/posthog` on branch `chore/enable-strict-script-versioning` at `935b7683660697bdc75c042c4c56828aeb036754`. The relevant Rust selector lives in `rust/feature-flags/src/flags/flag_matching.rs`: it walks condition groups in stored order, evaluates filters before rollout, applies `early_exit` only to `OutOfRolloutBound`, returns the first matching group, and resolves condition variant overrides. `flag_property_group.rs` preserves three aggregation states: field absent, explicit person (`null`), and explicit group index.
