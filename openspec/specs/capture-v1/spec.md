# Capture V1 Specification

## Purpose

The capture v1 capability is the **transport** that delivers PostHog analytics events over the
versioned ingestion endpoint `POST {host}/i/v1/analytics/events`. It is not a new public API: the
same events produced and enriched by `capture` (see the capture spec) are serialized into a v1
envelope, sent with Bearer-only authentication, and reconciled against a **per-event result
protocol** — every event in the batch comes back tagged `ok`, `warning`, `drop`, or `retry`, and
only the `retry`-tagged events are resent. This partial-retry model, the Bearer-only auth, and the
treatment of `429` as terminal are the ways v1 departs from the legacy `/batch/` transport.

This spec is the platform-agnostic **contract** every PostHog SDK's capture v1 implementation MUST
satisfy so that all SDKs stay on par. It is derived from the reference implementation `posthog-rs`
and the backend Rust type `rust/capture/src/v1/analytics/types.rs` in PostHog/posthog, and is
mirrored today by `posthog-go`, `posthog-node`, and `posthog-python` (each carries comments
pointing back at posthog-rs / the backend type). Each behavior here is the **canonical** choice;
where existing SDKs diverge — compression codec breadth, and posthog-node keeping `$ai_*` LLM
analytics events on the legacy transport — the requirement notes the allowed variation explicitly.

Capture v1 is a transport **mode** for the analytics-events pipeline, selected by configuration
alongside the legacy v0 transport. Event production, property enrichment, `before_send`, and
batching/queueing policy are unchanged and remain governed by the `capture` and `event-batcher`
specs; this spec governs only serialization, transport, and response reconciliation.
