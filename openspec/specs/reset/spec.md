...
reset(options?: boolean | { resetDeviceID?: boolean; bootstrap?: BootstrapConfig }): void
```
...
- The browser SDK also accepts an options object in place of the boolean, with `resetDeviceID` playing the same role as the boolean form plus an optional `bootstrap` object applied after the reset completes (see "Reset MAY accept bootstrap options to seed the next identity" below).
...
option (including the legacy boolean form), it SHALL continue to behave exactly as the
`Canonical reset behavior` requirement above describes — a fresh, un-seeded anonymous identity.
