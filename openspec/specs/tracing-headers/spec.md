...
| `X-POSTHOG-WINDOW-ID` | Current browser window/tab id | Request context `windowId`; emitted as event property `$window_id` |
...
| Values | Current distinct id, active session id, and window id where the platform has one. | Sanitized request-local context values. |
...
3. **Inject current values at request time.** Read the distinct id, session id, and window id when
...
4. **Header set.** Add `X-POSTHOG-DISTINCT-ID` when the current distinct id is present. Add
   `X-POSTHOG-SESSION-ID` when an active session id is present. Add `X-POSTHOG-WINDOW-ID` only on
   platforms with a browser-like window/tab id. Mobile SDKs without a window concept omit it.
...
2. **Read headers case-insensitively.** Recognize `X-POSTHOG-DISTINCT-ID`, `X-POSTHOG-SESSION-ID`,
   and `X-POSTHOG-WINDOW-ID` regardless of framework header casing or normalization.
...
4. **Store analytics context.** Valid values become request context `distinctId`, `sessionId`, and
   `windowId`. `sessionId` is also represented as `$session_id`, and `windowId` as `$window_id`, in
   context properties.
...
- current distinct id
- current session id
- current window/tab id where supported
...
- request-local PostHog context values (`distinctId`, `sessionId`, `windowId`, `properties`)
...
- **`capture`** — server captures may use context distinct id and context `$session_id` /
  `$window_id` when explicit values are absent.
...
- **`session-manager`** — supplies session and browser window ids where available.
...
- **AND** browser-like SDKs include `X-POSTHOG-WINDOW-ID: window-123`
- **AND** SDKs without a window concept omit `X-POSTHOG-WINDOW-ID`
...
- **AND** the request has `X-POSTHOG-WINDOW-ID: window-123`
...
- **AND** the event properties include `$window_id: window-123`
...
- **AND** context properties include `$session_id: header-session` and `$window_id: header-window`
- **WHEN** capture is called with distinct id "explicit-user" and properties `$session_id: explicit-session`, `$window_id: explicit-window`
- **THEN** the enqueued event distinct id is "explicit-user"
- **AND** the event properties keep `$session_id: explicit-session`
- **AND** the event properties keep `$window_id: explicit-window`
